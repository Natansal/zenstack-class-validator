//////////////////////////////////////////////////////////////////////////////////////////////
// DO NOT MODIFY THIS FILE                                                                  //
// This file is automatically generated by ZenStack CLI and should not be manually updated. //
//////////////////////////////////////////////////////////////////////////////////////////////

/* eslint-disable */

import { type SchemaDef, ExpressionUtils } from "@zenstackhq/orm/schema";
export class SchemaType implements SchemaDef {
   provider = {
      type: "postgresql",
   } as const;
   models = {
      User: {
         name: "User",
         fields: {
            createdAt: {
               name: "createdAt",
               type: "DateTime",
               attributes: [{ name: "@default", args: [{ name: "value", value: ExpressionUtils.call("now") }] }],
               default: ExpressionUtils.call("now"),
            },
            updatedAt: {
               name: "updatedAt",
               type: "DateTime",
               updatedAt: true,
               attributes: [{ name: "@updatedAt" }],
            },
            id: {
               name: "id",
               type: "String",
               id: true,
               attributes: [
                  { name: "@id" },
                  { name: "@default", args: [{ name: "value", value: ExpressionUtils.call("cuid") }] },
               ],
               default: ExpressionUtils.call("cuid"),
            },
            email: {
               name: "email",
               type: "String",
               attributes: [{ name: "@email" }],
            },
            name: {
               name: "name",
               type: "String",
               optional: true,
               attributes: [{ name: "@trim" }, { name: "@lower" }],
            },
            score: {
               name: "score",
               type: "Int",
               attributes: [
                  { name: "@gt", args: [{ name: "value", value: ExpressionUtils.literal(0) }] },
                  { name: "@lte", args: [{ name: "value", value: ExpressionUtils.literal(100) }] },
               ],
            },
            role: {
               name: "role",
               type: "Role",
            },
            posts: {
               name: "posts",
               type: "Post",
               array: true,
               relation: { opposite: "user" },
            },
         },
         idFields: ["id"],
         uniqueFields: {
            id: { type: "String" },
         },
      },
      Post: {
         name: "Post",
         fields: {
            createdAt: {
               name: "createdAt",
               type: "DateTime",
               attributes: [{ name: "@default", args: [{ name: "value", value: ExpressionUtils.call("now") }] }],
               default: ExpressionUtils.call("now"),
            },
            updatedAt: {
               name: "updatedAt",
               type: "DateTime",
               updatedAt: true,
               attributes: [{ name: "@updatedAt" }],
            },
            id: {
               name: "id",
               type: "String",
               id: true,
               attributes: [
                  { name: "@id" },
                  { name: "@default", args: [{ name: "value", value: ExpressionUtils.call("cuid") }] },
               ],
               default: ExpressionUtils.call("cuid"),
            },
            userId: {
               name: "userId",
               type: "String",
               attributes: [{ name: "@db.Uuid" }],
               foreignKeyFor: ["user"],
            },
            user: {
               name: "user",
               type: "User",
               attributes: [
                  {
                     name: "@relation",
                     args: [
                        { name: "fields", value: ExpressionUtils.array("String", [ExpressionUtils.field("userId")]) },
                        { name: "references", value: ExpressionUtils.array("String", [ExpressionUtils.field("id")]) },
                     ],
                  },
               ],
               relation: { opposite: "posts", fields: ["userId"], references: ["id"] },
            },
            title: {
               name: "title",
               type: "String",
               attributes: [
                  {
                     name: "@length",
                     args: [
                        { name: "min", value: ExpressionUtils.literal(1) },
                        { name: "max", value: ExpressionUtils.literal(200) },
                     ],
                  },
               ],
            },
            content: {
               name: "content",
               type: "String",
               optional: true,
            },
            published: {
               name: "published",
               type: "Boolean",
               attributes: [{ name: "@default", args: [{ name: "value", value: ExpressionUtils.literal(false) }] }],
               default: false,
            },
         },
         idFields: ["id"],
         uniqueFields: {
            id: { type: "String" },
         },
      },
   } as const;
   typeDefs = {
      CreatedUpdated: {
         name: "CreatedUpdated",
         fields: {
            createdAt: {
               name: "createdAt",
               type: "DateTime",
               attributes: [{ name: "@default", args: [{ name: "value", value: ExpressionUtils.call("now") }] }],
               default: ExpressionUtils.call("now"),
            },
            updatedAt: {
               name: "updatedAt",
               type: "DateTime",
               updatedAt: true,
               attributes: [{ name: "@updatedAt" }],
            },
         },
      },
   } as const;
   enums = {
      Role: {
         name: "Role",
         values: {
            USER: "USER",
            ADMIN: "ADMIN",
         },
      },
   } as const;
   authType = "User" as const;
   plugins = {};
}
export const schema = new SchemaType();
