...
/**
 * Creates a Nest mapped type from `dto` with the model's relation fields omitted.
 *
 ...
 * @param model - Model whose relations to omit.
 * @param dto - Source DTO class constructor.
 */
export function OmitRelations<Schema extends SchemaDef, M extends GetModels<Schema>, T extends object>(
   schema: Schema,
   model: M,
   dto: Type<T>,
) {
   return omitDtoKeys(dto, getRelationFieldsOfModel(schema, model));
}
