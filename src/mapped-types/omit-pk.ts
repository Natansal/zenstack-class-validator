...
/**
 * Creates a Nest mapped type from `dto` with the model's primary-key fields omitted.
 *
 ...
 * @param model - Model whose primary key to omit.
 * @param dto - Source DTO class constructor.
 */
export function OmitPK<Schema extends SchemaDef, M extends GetModels<Schema>, T extends object>(
   schema: Schema,
   model: M,
   dto: Type<T>,
) {
   return omitDtoKeys(dto, getPrimaryKeyFieldsOfModel(schema, model));
}
